...
## Example 4 - Spatial survival model
This the Leukemia data from Henderson et al (2003) (Henderson, R. and Shimakura, S. and Gorst, D., 2002, Modeling spatial variation in leukemia survival data, JASA, 97, 460, 965–972.)
...
Leuk$wbc <- scale(Leuk$wbc)
Leuk$tpi <- scale(Leuk$tpi)
summary(Leuk)
...
#CHECK
B.nweng <- neighbormatrix(nwEngland)
plot(nwEngland, col ="grey")
plotneighbors(nwEngland, add = TRUE)

##Areal data
form0 <- inla.surv(time, cens) ~ sex + age + wbc + tpi 


### model formulae with the spatial effect 
form <- update(
    form0, . ~ . +
               f(district, model = "bym2", graph = B.nweng))
...
plot(nwEngland)
#########
spplot(r$summary.random$district$mean, add = TRUE)



### mesh 
loc <- cbind(Leuk$xcoord, Leuk$ycoord)
nwseg <- inla.sp2segment(INLA::nwEngland)
...
ov <- over(SpatialPoints(prj$lattice$loc), INLA::nwEngland)
spat.sd[is.na(ov)] <- NA
spat.m[is.na(ov)] <- NA

### plot the spatial field
par(mfrow = c(1, 2), mar = c(5, 5, 5, 5))
image.plot(x = prj$x, y = prj$y, z = spat.m, main = "Spatial mean field")
plot(INLA::nwEngland, add = TRUE)
points(Leuk$x, Leuk$y, pch=13, col = "grey", cex = 0.2)

image.plot(x = prj$x, y = prj$y, z = spat.sd, main = "SD of the spatial field")
plot(INLA::nwEngland, add = TRUE)
points(Leuk$x, Leuk$y, pch=13, col = "grey", cex = 0.2)
```
